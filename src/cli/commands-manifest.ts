    ...
    parameters: [
      {
        ...
        type: 'string',
        required: false,
        description: 'URL to capture (required for --from live)',
      },
      {
        name: '--output',
        type: 'string',
        required: false,
        description: 'Output directory for capture data (required for --from live)',
      },
      {
        name: '--from',
        type: 'string',
        required: false,
        description: 'Source type: live (default) or code',
        default: 'live',
      },
      {
        name: '--input',
        type: 'string',
        required: false,
        description: 'Path to source code (--from code only)',
      },
      {
        name: '--framework',
        type: 'string',
        required: false,
        description: 'Test framework: playwright or cypress (--from code only, auto-detected)',
      },
      {
        name: '--headed',
        type: 'boolean',
        required: false,
        description: 'Run browser visibly (--from live only)',
      },
      {
        name: '--timeout',
        type: 'number',
        required: false,
        description: 'Navigation timeout in ms (--from live only)',
        default: 30000,
      },
      {
        name: '--no-screenshots',
        type: 'boolean',
        required: false,
        description: 'Disable screenshots (--from live only)',
      },
      {
        name: '--no-generate',
        type: 'boolean',
        required: false,
        description: 'Skip automatic spec generation (--from live only)',
      },
      {
        name: '--spec-output',
        type: 'string',
      ...
      },
      {
        ...
        type: 'boolean',
        required: false,
        description: 'Open a headed browser for human recording instead of autonomous agent',
      },
    ],
    modes: [
      {
        name: 'agent',
        description:
          'Autonomous agent explores and documents the application using Claude Agent SDK (default)',
      ...
      },
      {
        name: 'human',
        description:
          'Open a headed browser for human recording — browse the site and all traffic is captured',
  ...
  },
  {
    ...
    parameters: [
      ...
      },
      {
        ...
        type: 'string',
        required: true,
        description: 'Target URL to replay against',
      },
      { name: '--headed', type: 'boolean', required: false, description: 'Run browser visibly' },
      {
        ...
        type: 'string',
        required: false,
        description: 'Output directory for diff report',
      },
    ],
    examples: ['specify replay --capture ./captures/baseline --url http://localhost:3000'],
  },
  {
    name: 'compare',
    description:
      'Live side-by-side comparison of remote and local targets using parallel browser sessions',
    parameters: [
      ...
      { name: '--local', type: 'string', required: true, description: 'Local target URL' },
      {
        ...
        type: 'string',
        required: false,
        description: 'HTTP Basic Auth for remote (user:pass)',
      },
      {
        name: '--local-auth',
        type: 'string',
        required: false,
        description: 'HTTP Basic Auth for local (user:pass)',
      },
      {
        name: '--output',
        type: 'string',
        required: false,
        description: 'Output directory for comparison report',
      },
    ...
    parameters: [
      {
        ...
        type: 'string',
        required: true,
        description: 'Schema target: spec, report, or commands',
      },
    ],
  },
  {
