      ...
      rows: [
        {
          ...
          area_id: 'auth',
          behavior_id: 'login',
          content: 'click sign in then wait',
...
test('specMigrateId rewrites confidence, observation, and memory rows for the renamed id', async () => {
  ...
  try {
    ...
    const exitCode = await specMigrateId(
      { spec: specPath, oldId: 'auth/login', newId: 'auth/signin' },
      quietCtx(),
    );
    assert.equal(exitCode, 0);

    const confidence = JSON.parse(
      fs.readFileSync(path.join(dir, '.specify', 'confidence.json'), 'utf-8'),
    );
    ...
    const observations = yaml.load(
      fs.readFileSync(path.join(dir, 'specify.observations.yaml'), 'utf-8'),
    ) as { observations: Array<{ id: string; area_id?: string; behavior_id?: string }> };
...
test('specMigrateId rejects non-fully-qualified ids', async () => {
  ...
  try {
    const { specPath } = setupFixture(dir);
    const exitCode = await specMigrateId(
      { spec: specPath, oldId: 'login', newId: 'signin' },
      quietCtx(),
    );
...
test('a failed write to one store does not corrupt the others (tmp+rename per file)', async () => {
  ...
  try {
    ...
    assert.equal(
      ...
      originalConfidence,
      'destination file is untouched by the interrupted write',
    );
