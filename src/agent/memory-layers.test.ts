...
test('default paths root inside directory specs', () => {
  ...
  try {
    ...
    assert.equal(
      defaultObservationsPath(path.join(dir, 'spec')),
      path.join(dir, 'spec', 'specify.observations.yaml'),
    );
    assert.equal(
      defaultProjectMemoryPath(path.join(dir, 'spec')),
      path.join(dir, 'spec', 'SPECIFY.md'),
    );
  } finally {
    cleanup();
  }
