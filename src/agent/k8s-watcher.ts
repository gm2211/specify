...
export interface WatcherDeps {
   ...
   */
  findActiveVerify?: (target: {
    namespace: string;
    name: string;
    image?: string;
  }) => Promise<ActiveVerifyResult | undefined | null> | ActiveVerifyResult | undefined | null;
  /**
...
export async function startK8sWatcher(
  ...
  log(
    `[k8s-watcher] enabled namespaces=[${config.namespaces.join(',')}] selector=${config.labelSelector} resources=[${config.resources.join(',')}] inboxUrl=${config.inboxUrl} specPath=${config.specPath ?? 'none'} debounceMs=${config.debounceMs}\n`,
  );

  const fetchImpl = deps.fetchImpl ?? globalThis.fetch;
  const now = deps.now ?? (() => Date.now());
  const findActiveVerify =
    deps.findActiveVerify ?? ((target) => defaultFindActiveVerify(target, log));
  ...
  const stop = await watcher.start(async (ev) => {
    ...
    log(
      `[k8s-watcher] rollout detected ${ev.namespace}/${ev.name} kind=${ev.kind} image=${ev.image ?? 'unknown'} rv=${ev.resourceVersion ?? '?'}\n`,
    );
    ...
    if (config.debounceMs > 0) {
      ...
      if (lastTs !== undefined) {
        ...
        if (elapsedMs < config.debounceMs) {
          ...
          log(
            `[k8s-watcher] duplicate verify suppressed for ${ev.namespace}/${ev.name} (image=${ev.image ?? 'unknown'}, reason=debounce, last posted ${elapsedSec}s ago)\n`,
          );
          return;
        }
      }
    ...
    const active = await findActiveVerify({
      namespace: ev.namespace,
      name: ev.name,
      image: ev.image,
    });
    if (active) {
      ...
      log(
        `[k8s-watcher] duplicate verify suppressed for ${ev.namespace}/${ev.name} (image=${ev.image ?? 'unknown'}, reason=active-job ${active.id} status=${active.status})\n`,
      );
      return;
    }
...
function isTransientStreamClose(err: Error): boolean {
  ...
  if (msg.includes('ERR_HTTP2_STREAM_CANCEL') || msg.includes('ERR_HTTP2_SESSION_ERROR'))
    return true;
  return false;
}
...
function makeFireGate(log: (line: string) => void) {
  return (handler: (ev: RolloutEvent) => void) => {
    ...
    return (kind: string, event: 'add' | 'update', raw: unknown): void => {
      ...
      if (!ready) {
        log(
          `[k8s-watcher] event=${event} kind=${kind} ${ns}/${name} desired=${desired} ready=${status.readyReplicas ?? 0} updated=${status.updatedReplicas ?? 0} repl=${status.replicas ?? 0} obsGen=${status.observedGeneration ?? '?'}/gen=${obj.metadata?.generation ?? '?'} → skip\n`,
        );
        return;
      }
      ...
      log(
        `[k8s-watcher] event=${event} kind=${kind} ${ns}/${name} desired=${desired} ready=${status.readyReplicas ?? 0} updated=${status.updatedReplicas ?? 0} repl=${status.replicas ?? 0} obsGen=${status.observedGeneration ?? '?'}/gen=${obj.metadata?.generation ?? '?'} → ${decision}\n`,
      );
        ...
        for (const resource of config.resources) {
          ...
          const scheduleRestart = (reason: string, delayMs = 5_000) => {
            ...
            restartTimer = setTimeout(async () => {
              ...
              try {
                ...
                log(
                  `[k8s-watcher] informer reconnected ns=${namespace || 'all'} resource=${resource}\n`,
                );
              } catch (restartErr) {
                log(
            ...
            if (delayMs > 0) {
              log(
                `[k8s-watcher] informer ${reason} (${namespace || 'all'}/${resource}) — scheduling restart in ${delayMs / 1000}s\n`,
              );
            }
          };
          ...
          informer.on('error', (err: Error) => {
            ...
            if (isTransientStreamClose(err)) {
              ...
              log(
                `[k8s-watcher] informer stream closed (${namespace || 'all'}/${resource}): ${err.message} — reconnecting\n`,
              );
