...
export function buildDesignContext(
  spec: Spec,
  tokenExtraction?: DesignTokenExtraction,
): DesignContext {
...
export function renderDesignMarkdown(ctx: DesignContext, specSourceLabel: string): string {
  ...
  if (!ctx.tokens || ctx.tokens.tokens.length === 0) {
    lines.push(
      '_No design-token sources found (looked for `design-tokens.json`, `tokens.json`, and CSS custom properties)._',
    );
    lines.push('');
  } else {
    ...
    for (const [category, tokens] of [...byCategory.entries()].sort(([a], [b]) =>
      a.localeCompare(b),
    )) {
